...
class NDTwinDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # image
        image = Image.open(os.path.join(DATAPATH, 'CelebA_HQ_448_Final', image_path[image_id])).convert('RGB')  # (C, H, W)
...
class NDTwinVerificationDataset(Dataset):
    ...
    def _load_image(self, img_path):
        ...
        if img_path not in self.image_cache:
            full_path = os.path.join(self.data_path, 'CelebA_HQ_448_Final', img_path)
